"""
Support Vector Machine (SVM) implementation for food classification.
"""
...
class SVMClassifier(BaseFoodClassifier):
    """SVM Classifier with Optuna optimization."""
    ...
    def train(self, train_df: pd.DataFrame, val_df: pd.DataFrame, **kwargs) -> Dict[str, Any]:
        ...
        # If parameters provided in kwargs, set them
        if 'params' in kwargs:
    ...
    def evaluate(self, test_df: pd.DataFrame) -> Dict[str, float]:
        ...
        logger.info(f"SVM Test Accuracy: {acc:.4f}")
        logger.info(f"SVM Test F1 Score: {f1:.4f}")
        
        return {'accuracy': acc, 'f1_score': f1}
        
    def optimize(self, train_df: pd.DataFrame, val_df: pd.DataFrame, n_trials: int = 20) -> Dict[str, Any]:
        """Optimize SVM hyperparameters."""
        logger.info("Starting SVM optimization...")
        ...
        logger.info(f"Best SVM params: {self.best_params}")
